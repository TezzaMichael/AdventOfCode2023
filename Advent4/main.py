...
def parse(filename):
    ...
    for i in open(filename):
        ...
        lista1 = []
        lista2 = []
        for c in i.split("|")[0].split(" "):
            if c != "":
                lista1.append(c)
        for c in i.split("|")[1].split(" "):
            if c != "":
                lista2.append(c)
        
        n = 0
        for i in lista2:
            if i.strip() in lista1:
                n += 1
        listac.append(n)
    return listac

def Part1(filename):
    listac = parse(filename)
    tot = 0
    for n in listac:
        if n != 0:
            n = 2**(n-1)
...
def Part2(filename):
    ...
    for i in range(len(listac)):
        for j in range(listac[i]):
            if i + j + 1 < len(listac):
                listal[i+j+1] += listal[i]
    print(sum(listal))
if __name__ == "__main__":
    #Part1("./Advent4/input.txt")
    Part2("./Advent4/input.txt")